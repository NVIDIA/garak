...
const TechniqueIntentMatrix = ({
  ...
  // CSS grid: one label column + one fixed-width column per intent. Cells are
  // capped (not 1fr) so the matrix reads as a compact heatmap instead of
  // stretching each cell across the full panel width.
  const templateColumns = `minmax(7rem, max-content) repeat(${colLabels.length}, minmax(3.5rem, 5rem))`;
  ...
  return (
    <Stack gap="density-sm">
      ...
      >
        ...
        {colLabels.map((col) => (
          <Tooltip key={`h-${col}`} content={col}>
            <div
              role="columnheader"
              style={{
                padding: "4px 8px",
                textAlign: "center",
                // Long technique tags (e.g. "demon:T:Jailbreak") must wrap and
                // break within the capped column rather than overflow into the
                // neighbouring header.
                whiteSpace: "normal",
            ...
            {colLabels.map((col) => {
              ...
              const cellEl = (
                ...
                >
                  ...
                  <Text
                    kind="label/bold/sm"
                    style={cell ? { color: "var(--color-gray-900)" } : undefined}
                  >
                    {label}
                  </Text>
                </div>
