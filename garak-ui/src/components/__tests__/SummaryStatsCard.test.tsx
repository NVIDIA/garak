...
vi.mock("@kui/react", () => ({
  Notification: ({ status, slotHeading, slotSubheading, slotFooter, ...props }: any) => (
...
describe("SummaryStatsCard", () => {
  ...
  describe("Footer display logic", () => {
    it("shows footer with critical count when critical modules exist", () => {
      ...
      expect(screen.getAllByTestId("defcon-badge").filter(badge => 
        badge.getAttribute("data-defcon") === "1"
      )).toHaveLength(1);
    });
    ...
    it("shows footer with poor count when poor modules exist", () => {
      ...
      expect(screen.getByTestId("notification-footer")).toBeInTheDocument();
      expect(screen.getByText("3 Poor")).toBeInTheDocument();
      expect(screen.getAllByTestId("defcon-badge").filter(badge => 
        badge.getAttribute("data-defcon") === "2"
      )).toHaveLength(1);
    });
    ...
    it("shows both critical and poor counts when both exist", () => {
      ...
      expect(screen.getByText("2 Poor")).toBeInTheDocument();
