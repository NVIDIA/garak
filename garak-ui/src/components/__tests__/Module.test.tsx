...
vi.mock("./ProbesChart", () => ({
  __esModule: true,
  default: () => <div data-testid="probes-chart">Mock ProbesChart</div>,
}));
...
describe("Module", () => {
  ...
  it("toggles ProbesChart when header is clicked", () => {
    render(<Module module={mockModule} />);
    expect(screen.queryByTestId("probes-chart")).toBeNull();
  ...
  it("passes empty probes array if module.probes is undefined", () => {
    const incompleteModule = { ...mockModule, probes: undefined as any };
    render(<Module module={incompleteModule} />);
    fireEvent.click(screen.getByText("Bias"));
    expect(screen.getByTestId("probes-chart")).toBeInTheDocument();
  });
