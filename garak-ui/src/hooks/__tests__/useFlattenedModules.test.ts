...
const mockReport = {
  ...
  eval: {
    groupA: {
      _summary: { score: 0.5, group: "groupA", group_defcon: 2, doc: "", group_link: "", group_aggregation_function: "max" },
      probe1: {
        _summary: { probe_name: "p1", probe_score: 0.6, probe_severity: 2, probe_descr: "", probe_tier: 1 },
...
describe("useFlattenedModules", () => {
  ...
  it("includes 100% groups when show_100_pass_modules flag true", ()=>{
    ...
    report.eval.groupB = {
      _summary:{ score:1, group:"groupB", group_defcon:5, doc:"", group_link:"", group_aggregation_function:"max"},
    };
  ...
  it("skips detectors with high absolute score when show100Pass false", ()=>{
    const report = JSON.parse(JSON.stringify(mockReport));
    report.eval.groupA.probe1.detectorY = { absolute_score: 1.2 };
    const { result } = renderHook(()=> useFlattenedModules(report));
    const detectors = result.current[0].probes[0].detectors;
    expect(detectors.find((d:any)=> d.detector_name==="detectorY")).toBeUndefined();
  });
