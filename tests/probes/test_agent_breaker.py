...
def _make_probe(**overrides):
    ...
    with patch.object(AgentBreaker, "__init__", lambda self, config_root=None: None):
        ...
        probe.red_team_model_type = "inference_api.InferenceAPI"
...
class TestClassAttributes:
    ...
    def test_default_params_keys(self):
        expected_keys = {
            ...
            "end_condition", "num_generations", "max_calls_per_conv",
            "follow_prompt_cap", "generations", "agent_config_file",
            "max_attempts_per_tool", "success_threshold", "max_parallel_tools",
        }
...
class TestProbeOrchestration:
    ...
    def test_sequential_calls_each_tool(self):
        probe = _make_probe(max_parallel_tools=1)
        generator = MagicMock()
    ...
    def test_parallel_attacks_all_tools(self):
        probe = _make_probe(max_parallel_tools=4)
        generator = MagicMock()
        dummy_attempt = MagicMock()
        with patch.object(probe, "_setup_red_team_model"), \
             patch.object(probe, "_analyze_attackable_tools", return_value={
...
class TestAttackSingleTool:
    ...
    def test_stops_early_on_success(self):
        ...
        assert results[0].notes["tool_exploited"] is True
    ...
    def test_exhausts_max_attempts(self):
        ...
        assert len(final) == 1
        assert final[0].notes["tool_exploited"] is False
...
# ===========================================================================
# 7. _postprocess_attempt  (flag propagation across deepcopy)
# ===========================================================================
...
class TestPostprocessAttempt:
    ...
    def test_flags_propagated(self):
        probe = _make_probe()
        attempt = Attempt(prompt=Message(text="test"))
        attempt.notes["is_tool_final"] = True
        attempt.notes["tool_exploited"] = True

        with patch("garak.probes.base.Probe._postprocess_attempt") as mock_super:
            deep = copy.deepcopy(attempt)
            deep.notes.pop("is_tool_final", None)
            deep.notes.pop("tool_exploited", None)
            mock_super.return_value = deep

            result = probe._postprocess_attempt(attempt)

        assert result.notes["is_tool_final"] is True
        assert result.notes["tool_exploited"] is True
    ...
    def test_no_spurious_flags(self):
        ...
        assert "is_tool_final" not in result.notes
        assert "tool_exploited" not in result.notes


# ===========================================================================
# 8. _verify_attack_success  (JSON parsing edge cases)
# ===========================================================================
